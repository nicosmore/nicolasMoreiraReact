...
const Cart = () =>{
    ...
    const finalizarCompra = (datosComprador) => {
        let items = [];
        products.forEach(product => { items.push({name: product.name, precio: product.precio}) });
        ...
        const updateCollection = doc(db, "productos",product.id); //para actualizar stock
    ...
    return(
        ...
        <h4 className='d-flex justify-content-center border-top pt-3 fw-normal'>Carro de Compras</h4>
        <div className='mt-3 p-2 border bg-dark bg-opacity-10'>            
            {products.length===0
                ...
                :<div >
                    ...
                    <h4 className='m-2 shadowT mx-5 px-5 p-2 bg-opacity-50 bg-warning'>Total: $ {valorTotal}</h4>
                    <div className='m-2 shadowT  mx-5 px-5 p-2 border bg-light ' >
                        <h5 className='m-3 border-bottom '>Completar Datos Para Concretar Compra</h5>                        
                    ...
                    <Modal className='d-flex align-items-center' show={show}>
                        <div className='row-flex text-center px-5'>
                            ...
                            <Link to='/'>      
                                <button className='btn btn-primary m-2' onClick={clear}>Volver a Inicio</button>                            
                            </Link>
                        </div>        
